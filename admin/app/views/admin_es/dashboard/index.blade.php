								...
								@foreach ($not_approved_pages as $page)
									...
									<tr>
										<td>{{ App\Services\Helpers\MyHelper::format_date2($page->updated_at) }}
											:: {{ $page->category->name }} &gt;&gt; <a class="red" href="{{ URL::route('es-admin.pages.edit', $page->id) }}">{{ $page->name }} </a> </td>
									</tr>
						...
						<tbody>
							...
							@if (count($approved_pages))
								...
								@foreach ($approved_pages as $page)
									...
									<tr>
										<td>{{ App\Services\Helpers\MyHelper::format_date2($page->updated_at) }}
											:: {{ $page->category->name }} &gt;&gt; <a class="green" href="{{ URL::route('es-admin.pages.edit', $page->id) }}">{{ $page->name }} </a> </td>
									</tr>
