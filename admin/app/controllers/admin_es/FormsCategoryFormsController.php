...
class FormsCategoryFormsController extends BaseController {
	...
	{
		return View::make('admin_es.forms.index')
	...
	{
		return View::make('admin_es.forms.create')
			->with('category', FormCategory::find($category_id));
	}
	...
	public function store($category_id)
	{
		...
		if ($validation->passes())
		{
			...
			{
				$file = Input::file('pdf');
				$file->move('uploads/es/form', $file->getClientOriginalName());
			...
			return Redirect::route('es-admin.forms-category.forms.edit', array($category_id, $form->id));
		}
	...
	{
		return View::make('admin_es.forms.edit')->with('form', Form::find($id))
			->with('category', FormCategory::find($category_id));
	}
	...
	public function getDownload($id){
		...
		return Response::download('uploads/es/form/'. $id);
	}
